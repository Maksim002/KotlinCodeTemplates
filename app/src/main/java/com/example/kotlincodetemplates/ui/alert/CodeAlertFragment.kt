...
import kotlinx.android.synthetic.main.fragment_code_alert.*

class CodeAlertFragment : BaseFragment() {
    var url = "https://guides.codepath.com/android/using-dialogfragment"
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_code_alert, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        db.collection("alert_dialog").addSnapshotListener { value, error ->
            error?.message
            val result = value!!.documents[0].toObject(ModelElements::class.java)
            val sendPicture = try {
                requireArguments().getString("alert_dialog")
            } catch (e: Exception) {
                ""
            }
            when (sendPicture) {

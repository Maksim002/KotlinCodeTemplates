...
import kotlinx.android.synthetic.main.fragment_code_pdf.*

class CodePdfFragment : BaseFragment() {
    var url = "https://github.com/barteksc/AndroidPdfViewer"
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_code_pdf, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        db.collection("pdf_view").addSnapshotListener { value, error ->
            error?.message
            val result = value!!.documents[0].toObject(ModelElements::class.java)
            val sendPicture = try {
                requireArguments().getString("pdf_view")
            } catch (e: Exception) {
                ""
            }
            when (sendPicture) {

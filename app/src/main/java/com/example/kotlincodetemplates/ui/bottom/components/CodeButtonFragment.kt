...
import kotlinx.android.synthetic.main.fragment_code_button.*

class CodeButtonFragment : BaseFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        db.collection("button_view").addSnapshotListener { value, error ->
            error?.message
            val result = value!!.documents[0].toObject(ModelElements::class.java)
            val sendPicture = try {
                requireArguments().getString("button_view")
            } catch (e: Exception) {
                ""
            }
            when (sendPicture) {
